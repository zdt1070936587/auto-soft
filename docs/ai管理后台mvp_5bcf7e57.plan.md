---
name: AI管理后台MVP
overview: 从零搭建 AI 管理后台 MVP：用户对话即可生成动态 CRUD 与审批流。大模型走 OpenCode Go（管理员指定模型），运行时用元数据引擎 + warm-flow，不生成 Java/Vue 源码文件。
todos:
  - id: phase-0-scaffold
    content: 阶段0：建 docs/web/dev 骨架、规范文档、Spring Boot 4.1 多模块与 Vue3 空工程、Flyway
    status: pending
  - id: phase-1-auth
    content: 阶段1：JWT 登录、用户/角色/菜单 RBAC，太阳系登录页 + 后台 layout
    status: pending
  - id: phase-2-meta-runtime
    content: 阶段2：元数据引擎、dyn_ 动态表、通用 CRUD API、SchemaRenderer 与动态路由
    status: pending
  - id: phase-3-agent-go
    content: 阶段3：OpenCode Go 多协议路由、管理员指定模型、工作室 SSE + tool calling 生成 CRUD
    status: pending
  - id: phase-4-warm-flow
    content: 阶段4：warm-flow 集成、待办中心、Agent 生成单线审批并绑定实体
    status: pending
  - id: phase-5-harden
    content: 阶段5：鉴权/白名单/发布回滚/用量与日志、端到端验收与文档
    status: pending
isProject: false
---
...
- **目录**：前端 [`web/`](web/)，后端 [`dev/`](dev/)，设计文档 [`docs/`](docs/)。
...
- [`OpenCodeGoManager`](dev/auto-soft-agent) 按 `modelId` 选协议，对上层只暴露「对话 + tool calling + SSE」。
...
内置角色：`SUPER_ADMIN`（系统设置/模型）、`ADMIN`（用户角色菜单）、`DEVELOPER`（工作室）、`USER`（使用已发布功能）。
...
完整详细计划已输出到 [`docs/阶段0-脚手架与规范.md`](docs/阶段0-脚手架与规范.md)。

产出：

- [`docs/00-overview.md`](docs/00-overview.md)、[`docs/coding-standards.md`](docs/coding-standards.md)、[`docs/database.md`](docs/database.md)
- `dev` Maven 多模块可启动，健康检查 `/actuator/health`
- `web` Vite + Vue3 + Ant Design Vue + 路由骨架
- Flyway 空库可迁移；`.gitignore`、示例 `application-dev.yml`
...
- 操作日志（用户维护、发布、流程关键动作）
- [`docs/api.md`](docs/api.md)、[`docs/user-guide.md`](docs/user-guide.md)
...
阶段 0 → 1 → 2 → 3 → 4 → 5。阶段 2 是整条链路的底座，必须在 Agent 之前可手工跑通 CRUD；阶段 4 的流程工具建立在已发布实体之上。
