...
| 阶段 | 任务 | 状态 |
| --- | --- | --- |
| 0 | 建 `docs/` `web/` `dev/` 骨架、规范文档、Spring Boot 4.1 多模块与 Vue3 空工程、Flyway | 待开始 |
| 1 | JWT 登录、用户/角色/菜单 RBAC，太阳系登录页 + 后台 layout | 待开始 |
| 2 | 元数据引擎、`dyn_` 动态表、通用 CRUD API、SchemaRenderer 与动态路由 | 待开始 |
| 3 | OpenCode Go 多协议路由、管理员指定模型、工作室 SSE + tool calling 生成 CRUD | 待开始 |
| 4 | warm-flow 集成、待办中心、Agent 生成单线审批并绑定实体 | 待开始 |
| 5 | 鉴权/白名单/发布回滚/用量与日志、端到端验收与文档 | 待开始 |
