...
### 阶段 7：全局 AI 助手与记忆系统（P0～P3 约 17～23 人天）

完整 FAB 交互、pgvector 记忆表、Assistant 工具集、Consolidation 与验收见：[阶段7-全局AI助手与记忆系统.md](./阶段7-全局AI助手与记忆系统.md)

全站右下角浮动 AI 机器人（可收起），与 `/studio` 分离。支持：系统导航问答（返回可点击链接）、个人操作历史问答（基于 `sys_oper_log`）、跨会话用户画像记忆（PostgreSQL + **pgvector** + 定时 Consolidation）、通用闲聊。

分期：**P0** 浮动 UI + 导航；**P1** 操作历史；**P2** 向量记忆；**P3** 压缩与用户可管理记忆；**P4**（可选）页面访问埋点。

验收（P0+P1）：「用户管理在哪里」→ 链接跳转；「我昨天有没有新增过用户」→ 基于 oper log 如实回答并给出时间线。

### 阶段 8：助手操作 Copilot（P0 约 8～9 人天）

完整能力发现、ActionDraft、页面预填与验收见：[阶段8-助手操作Copilot.md](./阶段8-助手操作Copilot.md)；实现规格见 [spec/assistant-action-copilot.spec.md](./spec/assistant-action-copilot.spec.md)。

在阶段 7 全局助手基础上扩展：**自然语言描述操作 → 自动发现目标功能（动态 CRUD 零 Admin 配置）→ 缺项追问 → 确认并前往 → 目标页预填 → 用户点保存写库**。P0 覆盖系统用户新建 + 动态 CRUD 新建；不做 Action 管理配置页。

验收（P0）：「在用户模块添加用户 A，角色管理员」→ 追问用户名密码 → 跳转用户管理预填 → 保存成功；「添加一条请假，天数 3」→ 自动发现 runtime → 预填保存。
...
| 7 | 全局浮动 AI 助手、导航/操作问答、pgvector 跨会话记忆 | 文档已就绪 |
| 8 | 助手操作 Copilot：自动能力发现、ActionDraft、页面预填代操作 | 文档已就绪 |
