# 文档索引

本目录存放 AI 管理后台的设计与开发文档。代码在 `web/`（前端）与 `dev/`（后端）。

| 文档 | 说明 |
| --- | --- |
| [00-overview.md](./00-overview.md) | 产品定位、已确认决策、技术栈 |
| [分阶段开发计划.md](./分阶段开发计划.md) | MVP 分阶段总计划 |
| [阶段0-脚手架与规范.md](./阶段0-脚手架与规范.md) | 阶段 0 完整开发计划 |
| [阶段1-登录用户角色.md](./阶段1-登录用户角色.md) | 阶段 1 完整开发计划 |
| [阶段2-元数据引擎与动态运行时.md](./阶段2-元数据引擎与动态运行时.md) | 阶段 2 完整开发计划 |
| [阶段3-功能开发与OpenCode.md](./阶段3-功能开发与OpenCode.md) | 阶段 3 完整开发计划 |
| [阶段4-warm-flow审批.md](./阶段4-warm-flow审批.md) | 阶段 4 完整开发计划 |
| [阶段5-收口与MVP冻结.md](./阶段5-收口与MVP冻结.md) | 阶段 5 完整开发计划 |
| [coding-standards.md](./coding-standards.md) | 编码规范（Code Review 清单） |
| [database.md](./database.md) | 数据库约定与表规划 |
| [dev-setup.md](./dev-setup.md) | 本地启动与排障 |
| [api.md](./api.md) | 稳定接口一览 |
| [user-guide.md](./user-guide.md) | 使用手册 |
| [e2e-mvp.md](./e2e-mvp.md) | 端到端走查记录 |
| [spec/mvp.spec.md](./spec/mvp.spec.md) | 阶段 2～5 实现规格（As-Built） |
